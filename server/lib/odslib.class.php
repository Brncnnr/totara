                ...
                for($c=0; $c<=$ws->maxc; $c++) {
                    if (isset($ws->data[$r][$c])) {
                        ...
                        if (isset($cell->formula)) {
                            ...
                            $buffer .= '<table:table-cell office:value-type="date" office:date-value="' . \totara_core\strftime::format('%Y-%m-%dT%H:%M:%S', $cell->value) . '"'.$extra.'>'
                                     . $pretext . \totara_core\strftime::format('%Y-%m-%dT%H:%M:%S', $cell->value) . $posttext
                                     . '</table:table-cell>'."\n";
                        } else if ($cell->type == 'time') {
                            $timevalue  = 'PT' . \totara_core\strftime::format('%H', $cell->value) . 'H' . \totara_core\strftime::format('%M', $cell->value) . 'M' . '00S';
                            $buffer .= '<table:table-cell office:value-type="time" office:time-value="' . $timevalue . '"'.$extra.'>'
                                . $pretext . \totara_core\strftime::format('%H:%M', $cell->value) . $posttext
                                . '</table:table-cell>'."\n";
...
<office:document-meta xmlns:office="urn:oasis:names:tc:opendocument:xmlns:office:1.0"
    ...
    <office:meta>
        ...
        <meta:creation-date>'.\totara_core\strftime::format('%Y-%m-%dT%H:%M:%S').'</meta:creation-date>
