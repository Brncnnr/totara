...
class behat_general extends behat_base {
    ...
    /**
     * Ignore errors in logs so that we can test the expected error on the page.
     *
     * @Given I ignore :exception_message exceptions in log
     */
    public function i_ignore_exception_in_log($exception_message) {
        try {
            parent::look_for_exceptions();
        } catch (behat_log_exception $e) {
            if (strpos($e->getMessage(), $exception_message) === false) {
                throw new behat_log_exception($e->getMessage());
            }
        }
    }
