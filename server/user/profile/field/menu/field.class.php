...
class profile_field_menu extends profile_field_base {
    ...
    function validate_field_from_inputs(array $params): void {
        ...
        if (trim($params['data']) == "") {
            throw new moodle_exception("{$field}: You must pass a valid menu option.");
        }
        if (isset($params['data_format'])) {
            throw new moodle_exception("{$field}: data_format should not be passed for menu fields.");
        }

        $options = explode("\n", $this->field->param1);
        if (!in_array($params['data'], $options)) {
            throw new moodle_exception("{$field}: You must pass a valid menu option.");
        }
