...
class profile_field_base {
    ...
    public function edit_validate_field($usernew) {
        ...
        // Get input value.
        if (isset($usernew->{$this->inputname})) {
            if (is_array($usernew->{$this->inputname})) {
        ...
        if ($this->is_unique() && ($value !== '')) {
            $params = array('fieldid' => $this->field->id, 'value' => $value);
            // Include userid only if it's not NULL.
            $userid_condition = '';
            if (!empty($usernew->id)) {
                $userid_condition = " AND userid != :userid ";
                $params['userid'] = $usernew->id;
            }
    ...
    /**
     * Validate user input for user service.
     * @see \core_user\external\user_field_validator::validate_custom_field_from_inputs()
     *
     * @param array $params ['data' => 'data', 'data_format' => 0]
     * @return void
     */
    public function validate_field_from_inputs(array $params): void {
        if (isset($params['data_format'])) {
            try {
                $format = format::from_moodle($params['data_format']);
            } catch (\Exception $e) {
                $field = $this->field->shortname;
                throw new moodle_exception("{$field}: Unrecognised data_format: {$params['data_format']}.");
            }
        }
    }

    /**
     * Method to check if the current user has permission to set the value of this custom field.
     *
     * @return void
     */
    public function can_edit_locked_field(): void {
        if ($this->is_locked() && !has_capability('moodle/user:update', context_system::instance())) {
            $field = $this->field->shortname;
            throw new moodle_exception("{$field}: You do not have permission to set the value this locked custom field.");
        }
    }
}
